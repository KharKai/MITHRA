...
def chksum(data):
    ...
    for b in bytearray(data):
        checksum += int(b)
    return (((checksum & 0xffff) ^ 0xffff) + 1) # checksum correspond aux nombre de bits du message envoyé, 0xffff est
                                                # le nombre de byte max, et strange es le complément en byte pour
                                                # atteindre le nombre max de bytes
...
def packmsg(header, data):
    ...
    length = len(data)
    ba = header + packint(length) + bytearray(data, 'utf-8')  # Rmodif
    cs = chksum(ba)
...
class status:
    ...
    def __init__(self, raw):
        ...
        self.DetectorTemperature = raw[33]
        self.BoardTemperature = raw[34]
...
def printplaintextStatus(status):
    ...
    if status.MCS_DONE:
        ...
        strMCS_DONE = 'No'
    if status.PRECNT_REACHED:
        strPRECNT_REACHED = 'Yes'
    else:
        strPRECNT_REACHED = 'No'
    if status.PC5_PRESENT:
        strPC5_PRESENT = 'Yes'
    else:
        strPC5_PRESENT = 'No\n'

    strStatus = ('== MCA8000D status ==\n' +
                 'Device Id       : ' + str(status.DEVICE_ID) + '\n' +
                 'Firmware        : ' + str(status.Firmware) + '\n' +
                 'FPGA            : ' + str(status.FPGA) + '\n' +
                 'SerialNumber    : ' + str(status.SerialNumber) + '\n' +
                 'AccumulationTime: ' + str(status.AccumulationTime) + ' msec\n' +
                 'RealTime        : ' + str(status.RealTime) + ' msec\n' +
                 'FastCount       : ' + str(status.FastCount) + '\n' +
                 'SlowCount       : ' + str(status.SlowCount) + '\n' +
                 'GP Counter      : ' + str(status.GP_COUNTER) + '\n' +
                 'MCA_EN          : ' + strMCA_EN + '\n' +
                 'MCS_DONE        : ' + strMCS_DONE + '\n' +
                 'PRECNT_REACHED  : ' + strPRECNT_REACHED + '\n' +
                 'PC5_PRESENT     : ' + strPC5_PRESENT + '\n' +
                 'DPP_ECO         : ' + str(status.DPP_ECO) + '\n' +
                 '=====================')
    return strStatus
...
configParameters = {"RESC": "Reset Configuration",
                    ...
                    "MCAE": "MCA/MCS Enable",
                    "TECS": "Turn On/Off TEC Supply and Set the Temperature",
                    "TPEA": "Peaking time",
                    "TFLA": "Flat Top Width",
...
def printplaintextConfig(cfg):
    strConfig = '== MCA8000D Config ==\n'
    for k in cfg.keys():
        strConfig = strConfig + (configParameters[k] + ' : ' + cfg[k] + '\n')
    strConfig = strConfig + '=============\n'
    return strConfig
...
class device:
    ...
    def connect_xrf_spectrometer(self):
    ...
    def reqHWConfig(self):
        ...
        cfg = {}
        params = lcfgstr[0].decode(encoding="utf-8")
